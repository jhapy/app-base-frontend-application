 ...
 * @since 2019-02-14
 */
public abstract class JHapyAbstractDataProvider<T> extends AbstractBackEndDataProvider<T, Void>
  ...
  protected JHapyAbstractDataProvider(QueryGateway queryGateway) {
  ...
  protected Pageable getPageable(Query<T, Void> query) {
  ...
  protected <T, Void> Collection<Pageable.Order> createSpringSort(Query<T, Void> q) {
    ...
    List<Pageable.Order> orders =
        sortOrders.stream().map(JHapyAbstractDataProvider::queryOrderToSpringOrder).toList();
    if (orders.isEmpty()) {
