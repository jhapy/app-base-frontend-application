...
@SpringComponent
@UIScope
public class SecurityUserDataProvider extends DefaultDataProvider<SecurityUser, DefaultFilter>
    implements Serializable {

  @Autowired
  public SecurityUserDataProvider() {
    super(AppConst.DEFAULT_SORT_DIRECTION, AppConst.SECURITY_USER_SORT_FIELDS);
  }

  @Override
  protected PageDTO<SecurityUser> fetchFromBackEnd(
      Query<SecurityUser, DefaultFilter> query, Pageable pageable) {
    DefaultFilter filter = query.getFilter().orElse(DefaultFilter.getEmptyFilter());
    PageDTO<SecurityUser> page =
        SecurityServices.getSecurityUserService()
            .findAnyMatching(
