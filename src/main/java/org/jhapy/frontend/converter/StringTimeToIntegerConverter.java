...
public class StringTimeToIntegerConverter extends AbstractStringToNumberConverter<Integer> {
  ...
  public StringTimeToIntegerConverter(
      Integer emptyValue, ErrorMessageProvider errorMessageProvider) {
    super(emptyValue, errorMessageProvider);
  }

  @Override
  public Result<Integer> convertToModel(String value, ValueContext context) {
    if (StringUtils.isBlank(value)) return Result.ok(null);
    var duration =
        Arrays.stream(value.split(":")).mapToInt(Integer::parseInt).reduce(0, (n, m) -> n * 60 + m);
