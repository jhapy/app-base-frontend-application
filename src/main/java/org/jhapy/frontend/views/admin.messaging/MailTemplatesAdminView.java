...
public class MailTemplatesAdminView extends DefaultMasterView<MailTemplateDTO, DefaultFilter> {

  protected final transient QueryGateway queryGateway;
  private final JHapyAbstractDataProvider<MailTemplateDTO> dataProvider;

  public MailTemplatesAdminView(
      MyI18NProvider myI18NProvider, CommandGateway commandGateway, QueryGateway queryGateway) {
    super(
        "mailTemplate.",
        MailTemplateDTO.class,
        (DefaultDataProvider<MailTemplateDTO, DefaultFilter>) null,
        MailTemplateAdminView.class,
        myI18NProvider);
    this.queryGateway = queryGateway;
    this.dataProvider = new MailTemplateDataProvider(queryGateway);
  }

  protected Grid<MailTemplateDTO> createGrid() {
    grid = new Grid<>();
    ...
    grid.addColumn(MailTemplateDTO::getName).setKey("name");
    grid.addColumn(MailTemplateDTO::getMailAction).setKey("mailAction");
